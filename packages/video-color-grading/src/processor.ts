...
export class VideoColorGrading {
  ...
  private currentLUT: CubeLUT | null = null;
  ...
  async attachVideo(video: HTMLVideoElement): Promise<void> {
    // 切源前先停掉绑在旧 video 上的 RVFC/rAF 循环，避免 tick 闭包继续命中旧实例
    this.stop();
  ...
  setLUTIntensity(intensity: number): void {
    const next = Math.max(0, Math.min(100, intensity)) / 100;
    if (this.lutParams.intensity === next) return;
    this.lutParams = { ...this.lutParams, intensity: next };
    this.backend?.setLUTParams(this.lutParams);
  }
   ...
   */
  async ensureReadyForSource(width: number, height: number): Promise<void> {
    await this.ensureBackend();
    const backend = this.backend;
    if (!backend) throw new Error('Backend not available');
    // 用与目标同尺寸的 ImageBitmap 作为 placeholder source。
    // WebGPU 的 copyExternalImageToTexture 不接受没有 rendering context 的 OffscreenCanvas，
    // 但接受 ImageBitmap、HTMLCanvasElement、VideoFrame 等。
    ...
    // loadFromSource 内部走 disposeResources → 重建管线，会销毁 LUT/segMask 纹理。
    // 这里按 processor 缓存的状态把它们重新装回去，否则导出时调色全失效。
    if (this.currentLUT) {
