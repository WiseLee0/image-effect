...
const ImageTab = () => {
        ...
        if (shouldLoadDefaultLUTRef.current) {
          ...
          processor
            ...
            .then(() => {
              ...
              processor.setLUTSkinProtection(lutSkinRef.current);
            })
            .catch((err: Error) => {
              if (cancelled) return;
              console.warn("默认 LUT 加载失败:", err);
              setLutName(null);
            });
        }
      })
  ...
  const handleLUTPick = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      processor.setLUTSkinProtection(lutSkin);
...
const VideoTab = () => {
  ...
  useEffect(() => {
    const processor = new VideoColorGrading({
      ...
      enableSegmentationProfiling: true,
    });
    processorRef.current = processor;
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) console.warn("默认视频加载失败:", e);
      }
    })();
  ...
  const handleExport = async () => {
    ...
    if (!processor || !video || !videoLoaded) {
      console.warn("[handleExport] 跳过：未就绪", {
    ...
    try {
      ...
      console.log("[handleExport] exportVideo 返回 blob", {
        size: blob.size,
        type: blob.type,
      });
      const url = URL.createObjectURL(blob);
...
const LUTPanel = ({
  ...
  <div className="panel__group">
    ...
    <Slider label="肤色保护" value={skin} min={0} max={100} onChange={onSkin} />
  </div>
);
